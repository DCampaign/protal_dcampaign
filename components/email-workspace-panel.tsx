...
export function EmailWorkspacePanel(props: Props) {
  const { activePanel, onNavigate, recipientCount, batches, recipients, onRecipients, subject, onSubject, html, onHtml, batchSize, onBatchSize, interval, onInterval, scheduled, onSchedule, onSendNow, sendingNow, scheduleError, sendNowMessage, onPreview, previewOpen, onClosePreview, previewDevice, onPreviewDevice } = props;
  ...
  return (
    <section className="mx-auto max-w-7xl px-6 pb-16 pt-28 md:px-12">
      <div className="flex flex-col justify-between gap-5 border-b border-white/10 pb-7 sm:flex-row sm:items-end">
        ...
        <div className="rounded-xl border border-brand/30 bg-[#24130d] px-4 py-3 text-sm font-bold text-brand-light">{recipientCount} / 100 recipients</div>
      </div>
      ...
      {activePanel === 'delivery' && <div className="mt-7 grid gap-6 lg:grid-cols-[minmax(0,1fr)_340px]">
        <div className="rounded-3xl border border-white/10 bg-[#171719] p-6 sm:p-8"><h2 className="font-display text-2xl font-extrabold">Delivery settings</h2><p className="mt-2 text-sm text-white/55">Messages are sent in measured batches to keep campaign delivery controlled.</p><div className="mt-8 grid gap-5 sm:grid-cols-2"><label className="text-sm font-bold text-white/75">Emails per batch<input type="number" min="1" max="25" value={batchSize} onChange={(event) => onBatchSize(Math.min(25, Math.max(1, Number(event.target.value))))} className="mt-2 h-12 w-full rounded-xl border border-white/10 bg-[#111113] px-4 text-white outline-none focus:border-brand" /></label><label className="text-sm font-bold text-white/75">Interval in minutes<input type="number" min="30" value={interval} onChange={(event) => onInterval(Math.max(30, Number(event.target.value)))} className="mt-2 h-12 w-full rounded-xl border border-white/10 bg-[#111113] px-4 text-white outline-none focus:border-brand" /></label></div><p className="mt-3 text-xs text-white/45">Current Hostinger plan: up to 25 emails every 30 minutes, maximum 100 emails in 24 hours.</p>{scheduleError && <p className="mt-4 rounded-xl border border-red-400/20 bg-red-400/10 px-4 py-3 text-sm text-red-200">{scheduleError}</p>}{sendNowMessage && <p className="mt-4 rounded-xl border border-brand/25 bg-[#24130d] px-4 py-3 text-sm text-brand-light">{sendNowMessage}</p>}<div className="mt-8 flex flex-wrap gap-3"><button type="button" disabled={!canSchedule || scheduled || sendingNow} onClick={onSchedule} className="inline-flex h-12 items-center gap-2 rounded-xl border border-white/15 px-5 text-sm font-extrabold text-white hover:border-brand/60 disabled:cursor-not-allowed disabled:opacity-40"><CalendarClock size={17} />{scheduled ? 'Campaign scheduled' : 'Schedule campaign'}</button><button type="button" disabled={!canSchedule || scheduled || sendingNow} onClick={onSendNow} className="inline-flex h-12 items-center gap-2 rounded-xl bg-brand px-5 text-sm font-extrabold text-white hover:bg-brand-dark disabled:cursor-not-allowed disabled:opacity-40"><Send size={17} />{sendingNow ? 'Sending first batch…' : 'Send now'}</button></div><p className="mt-3 text-xs text-white/40">Send now delivers the first allowed batch immediately. Any remaining recipients continue automatically on the selected schedule.</p></div>
        <aside className="rounded-3xl border border-brand/25 bg-[#24130d] p-6"><Clock3 size={22} className="text-brand-light" /><p className="mt-6 text-xs font-extrabold uppercase tracking-[0.2em] text-brand">SENDING PLAN</p><p className="mt-3 font-display text-3xl font-extrabold">{batchSize} emails</p><p className="mt-1 text-sm text-white/60">every {interval} minutes</p><div className="my-6 border-t border-brand/20" /><p className="text-sm leading-6 text-white/60">This creates up to {batches.length || 0} batches and never exceeds the 100-email daily limit.</p></aside>
      </div>}
