...
const publicPaths = ['/client/login', '/client/forgot-password', '/client/reset-password', '/crm/login', '/auth/callback'];

export async function proxy(request: NextRequest) {
  let response = NextResponse.next({ request });
  const pathname = request.nextUrl.pathname;
  const isPublic = publicPaths.some((path) => pathname.startsWith(path));
  const protectedRoute = pathname.startsWith('/client/') || pathname.startsWith('/admin/') || pathname === '/crm' || pathname.startsWith('/crm/');
...
export const config = { matcher: ['/client/:path*', '/admin/:path*', '/crm/:path*', '/crm', '/auth/:path*'] };
