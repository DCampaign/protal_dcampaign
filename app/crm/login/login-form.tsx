...
export function CrmLoginForm() {
  ...
  return <form action={action} className="mt-8 space-y-5">
    ...
    <label className="block"><span className="mb-2 block text-sm font-bold text-white/70">Password</span><span className="flex h-13 items-center gap-3 rounded-xl border border-white/10 bg-white/[.04] px-4 focus-within:border-brand/70 focus-within:ring-2 focus-within:ring-brand/15"><LockKeyhole size={18} className="text-white/35"/><input name="password" type="password" autoComplete="current-password" minLength={8} required placeholder="Enter your password" className="h-full w-full bg-transparent text-sm text-white outline-none placeholder:text-white/25"/></span></label>
