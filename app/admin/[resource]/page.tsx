...
export default async function AdminResourcePage({ params }: { params: Promise<{ resource: string }> }) {
  ...
  return <><div className="flex flex-col justify-between gap-5 sm:flex-row sm:items-end"><div><p className="text-sm font-extrabold uppercase tracking-widest text-brand">Administration</p><h1 className="mt-3 font-display text-4xl font-extrabold">{config.title}</h1><p className="mt-3 max-w-2xl text-base leading-7 text-white/50">{config.description}</p></div><Link href={`/admin/${resource}/new`} className="inline-flex min-h-12 items-center justify-center rounded-xl bg-brand px-5 text-sm font-extrabold hover:bg-brand-dark">＋ Add {config.title.slice(0, -1)}</Link></div>{rows.length ? <div className="mt-8 overflow-hidden rounded-2xl border border-white/8"><div className="overflow-x-auto"><table className="w-full min-w-[760px] text-left"><thead className="bg-white/[0.04] text-xs uppercase tracking-wider text-white/45"><tr>{config.columns.map(([, label]) => <th key={label} className="p-4">{label}</th>)}</tr></thead><tbody>{rows.map((row, index) => <tr key={String(row.id ?? index)} className="border-t border-white/8 text-sm">{config.columns.map(([key]) => <td key={key} className="p-4 capitalize text-white/65 first:font-bold first:text-white">{valueFor(row, key)}</td>)}</tr>)}</tbody></table></div></div> : <div className="mt-8 rounded-2xl border border-dashed border-white/12 p-8"><h2 className="font-display text-xl font-bold">Nothing here yet</h2><p className="mt-2 text-base text-white/50">Records will appear as the team begins using this workflow.</p></div>}</>;
}
