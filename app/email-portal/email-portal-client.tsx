...
export function EmailPortalClient() {
  ...
  const emails = useMemo(() => recipients.split(/[\s,;]+/).map((x) => x.trim().toLowerCase()).filter((x, i, a) => /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(x) && a.indexOf(x) === i).slice(0, 450), [recipients]);
  ...
  return (
    <section className="mx-auto max-w-7xl px-6 pb-16 pt-28 md:px-12">
      ...
      <div className="mt-8 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <DashboardMetric icon={Mail} label="Daily capacity" value={recipientCount + ' / 450'} detail="Recipients ready today" />
      ...
      <div className="mt-6 grid gap-6 lg:grid-cols-[1.25fr_.75fr]">
        <section className="rounded-3xl border border-white/10 bg-[#171719] p-6 sm:p-7">
          ...
          <p className="mt-3 max-w-xl text-sm leading-6 text-white/55">Choose a template, add recipients, preview the email, then schedule controlled batches of up to 50 emails every 30 minutes.</p>
          <div className="mt-6 grid gap-3 sm:grid-cols-3">
            <QuickAction icon={Code2} title="Choose template" text="Start with a proven structure." onClick={onCreate} />
            <QuickAction icon={Users} title="Import audience" text="Add up to 450 contacts." onClick={onAudience} />
        ...
        <section className="rounded-3xl border border-white/10 bg-[#111113] p-6 sm:p-7">
          ...
          <div className="mt-6 rounded-2xl border border-white/8 bg-[#171719] p-4">
            <div className="flex items-center justify-between text-sm font-bold">
              ...
              <span className="text-brand-light">{recipientCount} / 450</span>
            </div>
