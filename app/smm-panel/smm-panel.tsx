...
import { ArrowUpRight, Boxes, CheckCheck, ChevronRight, CircleHelp, ClipboardList, Coins, Link2, LayoutDashboard, LockKeyhole, LogOut, MessageCircle, PackageCheck, PlugZap, Plus, RefreshCw, Search, ShoppingBag, Sparkles, UsersRound } from 'lucide-react';
...
const tabs = [
  ...
  { id: 'refill', label: 'Refill', icon: RefreshCw },
  { id: 'funds', label: 'Add funds', icon: Coins },
  { id: 'api', label: 'API', icon: Link2 },
  { id: 'mass', label: 'Mass order', icon: PackageCheck },
  { id: 'referrals', label: 'Refer & Earn', icon: UsersRound },
  { id: 'updates', label: 'Updates', icon: Sparkles },
] as const;
type Tab = typeof tabs[number]['id'];
const empty: PanelData = { providers: [], services: [], orders: [] };
const title: Record<Tab, string> = { overview: 'Your social workspace.', new: 'Create an order.', services: 'Find the right service.', orders: 'Every order. In one place.', providers: 'Connect your providers.', tickets: 'Support tickets.', refill: 'Refill requests.', funds: 'Add funds.', api: 'Developer API.', mass: 'Mass order.', referrals: 'Refer & earn.', updates: 'Panel updates.' };
  ...
  return <section className={styles.root}>
    ...
    <div className={styles.workspace}>
      ...
      <div className={styles.content}>
        ...
        {(['tickets', 'refill', 'funds', 'api', 'mass', 'referrals', 'updates'] as Tab[]).includes(tab) && <Card><p className={styles.kicker}>SMM WORKSPACE</p><h2>{title[tab]}</h2><p className={styles.muted}>This workspace is ready for your team’s workflow and will stay connected to your service catalog and orders.</p><div className={styles.message}>{tab === 'tickets' ? 'Create and track support requests from one place.' : tab === 'refill' ? 'Submit refill requests for eligible orders.' : tab === 'funds' ? 'Configure funding instructions and payment references.' : tab === 'api' ? 'Review API access details for approved integrations.' : tab === 'mass' ? 'Prepare multiple service orders in one submission.' : tab === 'referrals' ? 'Share your referral link and track rewards.' : 'View service announcements and panel updates.'}</div><button className={styles.primary} onClick={() => switchTab(tab === 'mass' ? 'new' : 'overview')}>{tab === 'mass' ? 'Start an order' : 'Back to overview'}<ArrowUpRight size={17} /></button></Card>}
      </div>
    </div>
  </section>;
}
